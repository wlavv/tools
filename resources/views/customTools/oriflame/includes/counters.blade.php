<div class="col-lg-12">
    <div class="navbar navbar-light customPanel">
        <div class="row">
            <div class="col-lg-2 col-md-3 col-sm-6 col-6">
                <div class="card-counter primary">
                    <div style="width: 100px;float: left;"> <i class="fa fa-code-fork"></i> </div>
                    <div style="width: calc( 100% - 100px); float: left;padding: 10px 0;">
                        <div class="count-numbers">17</div>
                        <br><div class="count-name">Orders</div>
                    </div>
                </div>
            </div>
            <div class="col-lg-2 col-md-3 col-sm-6 col-6">
                <div class="card-counter info">
                    <div style="width: 100px;float: left;"> <i class="fa fa-boxes-packing"></i> </div>
                    <div style="width: calc( 100% - 100px); float: left;padding: 10px 0;">
                        <span class="count-numbers">12</span>
                        <br><span class="count-name">Open</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-2 col-md-3 col-sm-6 col-6">
                <div class="card-counter success">
                    <div style="width: 100px;float: left;"> <i class="fa fa-truck-fast"></i> </div>
                    <div style="width: calc( 100% - 100px); float: left;padding: 10px 0;">
                        <span class="count-numbers">2312</span>
                        <br><span class="count-name">Shipped</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-2 col-md-3 col-sm-6 col-6">
                <div class="card-counter danger">
                    <div style="width: 100px;float: left;"> <i class="fa fa-xmark"></i> </div>
                    <div style="width: calc( 100% - 100px); float: left;padding: 10px 0;">
                        <span class="count-numbers">104</span>
                        <br><span class="count-name">Cancelled</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-2"></div>
            <div class="col-lg-2">
                <button class="btn btn-success float-end" type="button" style="padding: 12px" onclick="newOrder()">
                    <i class="fa-solid fa-cart-plus" style="font-size: 30px;"></i>
                    <br>
                    <div style="margin-top: 10px;">NEW ORDER</div>
                </button>                        
            </div>
        </div>
    </div>           
</div>