...
class MissingHousingPanel extends AbstractIssuePanel implements IssuePanelInterface
{
    public function resolve(array $context = []): IssuePanelResult
    {
        if (!CatalogTable::exists('catalog_core_products')) {
            return new IssuePanelResult(key: 'missing_housing', title: 'Produtos sem Housing');
        }

        $query = DB::table('catalog_core_products as p')
            ->where(function ($q) {
                $q->whereNull('p.housing')->orWhere('p.housing', '');
            });
        ...
        $items = $query->select('p.id', 'p.name', 'p.reference', 'p.status')
            ...
            ->map(fn ($product) => [
                ...
                'subtitle' => 'Sem housing/localização',
                'badge' => $product->status ?: 'draft',
                'url' => $this->productUrl($product->id),
            ])->toArray();

        return new IssuePanelResult(key: 'missing_housing', title: 'Produtos sem Housing', count: $count, items: $items);
    }
}
