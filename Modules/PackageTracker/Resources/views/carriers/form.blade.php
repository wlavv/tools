...
<div class="card shadow-sm border-0">
    <div class="card-body">
        <form id="lsg-form" method="POST" action="{{ $editing ? route('package_tracker.carriers.update', $carrier) : route('package_tracker.carriers.store') }}" class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label">API Key</label><input name="api_key" value="{{ old('api_key') }}" class="form-control"></div>
            <div class="col-md-3"><label class="form-label">API Secret</label><input name="api_secret" value="{{ old('api_secret') }}" class="form-control"></div>
