...
@php
    ...
    $panelRoute = fn($task) => route('project_manager.tasks.panel', $task->id);
    $matrixRoute = fn($task) => route('project_manager.tasks.priority_matrix', $task->id);
    $blockRoute = fn($task) => route('project_manager.tasks.block', $task->id);
...
<div class="lsg-content pm-wrap">
    <div class="pm-shell">
        <div class="pm-page-intro">
            <div>
                <div class="pm-page-kicker">Productivity global</div>
                <div class="pm-page-lead">Mostra apenas tasks dos milestones atualmente ativos em cada projeto aberto.</div>
            </div>
            <div class="pm-context-pill"><i class="fa-solid fa-layer-group"></i> {{ count($activeMilestones) }} milestones ativos</div>
        </div>

        <div class="pm-card mb-3">
            <div class="pm-section-bar">
                <div>
                    <div class="pm-card-title"><i class="fa-solid fa-flag-checkered"></i> Milestones em trabalho</div>
                    <div class="pm-card-subtitle mb-0">Cada projeto aberto contribui com o seu milestone atual.</div>
                </div>
            </div>
            <div class="pm-milestone-strip">
                @forelse($activeMilestones as $milestone)
                    <a class="pm-milestone-chip" href="{{ route('project_manager.projects.roadmap.index', $milestone->project_id) }}">
                        <span>{{ $milestone->project_name }}</span>
                        <strong>{{ $milestone->title }}</strong>
                    </a>
                @empty
                    <div class="pm-empty">Não existem milestones ativos nos projetos abertos.</div>
                @endforelse
            </div>
        </div>

        <div class="pm-kanban pm-kanban--trello mb-3">
            <div class="pm-kanban-column" data-pm-panel="execution">
                ...
                @forelse($executionTasks as $task)
                    <article class="pm-kanban-card" data-pm-task-card="{{ $task->id }}">
                        ...
                        <div class="pm-kanban-card-meta"><strong>{{ $task->project_name }}</strong> · {{ $task->milestone_title }}</div>
                        ...
                        <div class="pm-kanban-actions">
                            ...
                            <button type="button" class="pm-btn pm-btn--compact pm-btn--danger" data-pm-block-open data-block-url="{{ $blockRoute($task) }}" data-task-title="{{ e($task->title) }}"><i class="fa-solid fa-ban"></i> Bloquear</button>
                ...
                @empty
                    <div class="pm-empty">Sem tarefas em execução nos milestones atuais.</div>
                @endforelse
            </div>
                ...
                @forelse($nextTasks as $task)
                    <article class="pm-kanban-card" data-pm-task-card="{{ $task->id }}">
                        ...
                        <div class="pm-kanban-actions">
                            ...
                            <button type="button" class="pm-btn pm-btn--compact pm-btn--danger" data-pm-block-open data-block-url="{{ $blockRoute($task) }}" data-task-title="{{ e($task->title) }}"><i class="fa-solid fa-ban"></i></button>
                            <a class="pm-btn pm-btn--compact pm-btn--warning" href="{{ route('project_manager.projects.tasks.edit', [$task->project_id, $task->id]) }}"><i class="fa-solid fa-pencil"></i></a>
                        </div>
                    </article>
                @empty
                    <div class="pm-empty">Sem tarefas seguintes nos milestones atuais.</div>
                @endforelse
            </div>
            ...
            <div class="pm-kanban-column" data-pm-panel="blocked">
                ...
                @empty
                    <div class="pm-empty">Sem bloqueios nos milestones atuais.</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="pm-grid-2">
            <div class="pm-card">
                <div class="pm-section-bar">
                    <div>
                        <div class="pm-card-title"><i class="fa-solid fa-chart-gantt"></i> Gantt global</div>
                        <div class="pm-card-subtitle mb-0">Apenas tasks pertencentes aos milestones ativos.</div>
                    </div>
                </div>
                <div class="pm-gantt">
                    @forelse($ganttTasks as $task)
                        ...
                        <div class="pm-gantt-row">
                            <div class="pm-gantt-label">{{ Str::limit($task->project_name . ' · ' . $task->title, 35) }}</div>
            ...
            <div class="pm-card">
                <div class="pm-section-bar">
                    <div>
                        <div class="pm-card-title"><i class="fa-solid fa-table-cells-large"></i> Matriz de Eisenhower global</div>
                        <div class="pm-card-subtitle mb-0">Alimentada pelos milestones ativos de todos os projetos abertos.</div>
                    </div>
    ...
    if (modal && form) {
        document.querySelectorAll('[data-pm-block-open]').forEach(function(button){
            button.addEventListener('click', function(){
                form.action = this.dataset.blockUrl;
