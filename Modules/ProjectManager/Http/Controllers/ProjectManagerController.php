...
class ProjectManagerController extends Controller{
    ...
    public function __construct(protected ProjectManagerService $service){
        $this->setIndexPage('project_manager', 'project_manager.index');
        $this->middleware('auth');
    }

    public function index(): View{
        return $this->view('project-manager::index', ['groups' => $this->service->getGroups(), 'stats' => $this->service->getStats()]);
    }

    public function create(): View{
        return $this->view('project-manager::pages.create', ['statuses' => config('project-manager.project_statuses', []), 'projects' => ProjectManager::where('id_parent',0)->orderBy('name')->get()]);
    }
    ...
    public function edit(ProjectManager $project): View{
        return $this->view('project-manager::pages.edit', ['project' => $project, 'statuses' => config('project-manager.project_statuses', []), 'projects' => ProjectManager::where('id_parent',0)->where('id','!=',$project->id)->orderBy('name')->get()]);
    }
