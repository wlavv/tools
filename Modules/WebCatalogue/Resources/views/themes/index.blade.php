...
<div class="wc-card">
    <div class="wc-list-toolbar"><div><span class="wc-eyebrow"><i class="fa-solid fa-palette"></i> Visual identity</span><h3>Themes</h3><p class="wc-muted">Store typography, colours, logos and frontend styling presets.</p></div></div>
    ...
    @forelse($items as $item)
        <div class="wc-rich-card">
            ...
            <div class="wc-rich-body">
                <div class="wc-rich-title"><h4><a href="{{ route('webcatalogue.themes.show', $item) }}">{{ $item->name ?? 'Theme #'.$item->id }}</a></h4><span class="wc-badge wc-status-{{ $item->status ?? 'active' }}">{{ $item->status ?? 'active' }}</span>@if($item->is_default)<span class="wc-badge wc-status-active">Default</span>@endif</div>
            ...
            <div class="wc-rich-actions"><a class="wc-action-link" href="{{ route('webcatalogue.themes.show', $item) }}"><i class="fa-solid fa-eye"></i> View</a><a class="wc-action-link" href="{{ route('webcatalogue.themes.edit', $item) }}"><i class="fa-solid fa-pencil"></i> Edit</a></div>
        </div>
    @empty
