    ...
    <div class="wc-card">
        ...
        <form id="lsg-form" method="POST" enctype="multipart/form-data" action="{{ $action }}">
            ...
            <div class="wc-form-panel"><div class="wc-form-panel-head"><span class="wc-form-panel-icon"><i class="fa-solid fa-cube"></i></span><div><h4>Scene identity</h4></div></div><div class="wc-form-grid">
                ...
                <div class="wc-field"><label>Catalogue</label><select name="id_catalogue"><option value="">Store-wide environment</option>@foreach($catalogues ?? [] as $catalogue)<option value="{{ $catalogue->id }}" data-store="{{ $catalogue->id_store }}" @selected((string)old('id_catalogue', $item->id_catalogue ?? request('id_catalogue', '')) === (string)$catalogue->id)>{{ $catalogue->name }}</option>@endforeach</select><div class="wc-upload-hint">Optional. Use this for thematic catalogue environments.</div></div>
