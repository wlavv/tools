...
<div class="wc-editor-layout">
    <div>
        <div class="wc-detail-hero wc-detail-hero-store">
            <div>
                ...
                <h2>{{ $item->name ?? $item->title ?? $item->reference ?? 'Store' }}</h2>
        ...
        <div class="wc-detail-grid">
            <div class="wc-info-card"><i class="fa-solid fa-circle-info"></i><span>Code</span><strong>{{ $item->code ?? '—' }}</strong></div>
            <div class="wc-info-card"><i class="fa-solid fa-circle-info"></i><span>Domain</span><strong>{{ $item->domain ?? '—' }}</strong></div>
            <div class="wc-info-card"><i class="fa-solid fa-circle-info"></i><span>Status</span><strong>{{ $item->status ?? '—' }}</strong></div>
            <div class="wc-info-card"><i class="fa-solid fa-circle-info"></i><span>Logo Path</span><strong>{{ $item->logo_path ?? '—' }}</strong></div>
        </div>
    ...
    <aside class="wc-preview-panel">
        ...
        <div class="wc-preview-card">
            <div class="wc-preview-media"><i class="fa-solid fa-store wc-preview-icon"></i></div>
            <div class="wc-preview-body"><h4>Store preview</h4><p class="wc-muted">Visual summary of this record.</p></div>
        </div>
        <div class="wc-preview-card"><div class="wc-preview-body"><h4>Actions</h4><div class="wc-actions-row"><a class="wc-action-link" href="{{ route('webcatalogue.stores.edit', $item) }}"><i class="fa-solid fa-pencil"></i> Edit</a><a class="wc-action-link" href="{{ route('webcatalogue.stores.index') }}"><i class="fa-solid fa-angle-left"></i> Back</a></div></div></div>
    </aside>
</div>
</div>
@endsection
