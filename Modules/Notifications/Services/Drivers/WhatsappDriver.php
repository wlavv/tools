...
class WhatsappDriver extends AbstractDriver
{
    ...
    {
        if (!$recipient->phone) {
            return ['provider' => 'whatsapp', 'status' => 'failed', 'error_message' => 'Recipient phone missing.'];
        }

        $provider = NotificationProviderConfig::query()->where('channel', 'whatsapp')->where('enabled', true)->first();
        ...
        if ($providerName === 'twilio') {
            ...
            if (!$sid || !$token || !$from) {
                return ['provider' => 'twilio', 'status' => 'failed', 'error_message' => 'Twilio WhatsApp credentials missing.'];
            }
            $url = 'https://api.twilio.com/2010-04-01/Accounts/' . $sid . '/Messages.json';
            $response = Http::asForm()->withBasicAuth($sid, $token)->post($url, [
                'To' => str_starts_with($recipient->phone, 'whatsapp:') ? $recipient->phone : 'whatsapp:' . $recipient->phone,
        ...
        if (!$webhook) {
            return ['provider' => 'generic_webhook', 'status' => 'failed', 'error_message' => 'WhatsApp webhook not configured.'];
        }
        ...
        $response = Http::withHeaders(array_filter([
            ...
            'to' => $recipient->phone,
            'message' => $channelPayload['message'] ?? ($notification->title . "\n" . $notification->message),
        ]);
